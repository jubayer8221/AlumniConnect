...
function ensureRegistrations(): EventRegistration[] {
  let regs = localStorageService.get<EventRegistration[]>(
...
export const mockEventService: EventService = {
  async getAll(
    ...
    const filtered = all.filter((e) =>
      matchesEventFilters(e, params?.filters, params?.search),
    );
  ...
  async getById(id: string): Promise<ApiResponse<EventItem>> {
    ...
    if (!found)
      return {
        ...
        data: null as unknown as EventItem,
      };
  ...
  async register(
    ...
    if (!event)
      return {
        success: false,
        message: "Event not found",
        data: null as unknown as EventRegistration,
      };
  ...
  async cancelRegistration(
    ...
    if (idx === -1)
      return { success: false, message: "Registration not found", data: null };
  ...
  async getRegistrations(
    ...
    return {
      ...
      data: regs.filter((r) => r.eventId === event.id),
    };
  },
  ...
  async getMyRegistrations(
    ...
    return {
      success: true,
      message: "My registrations retrieved",
      data: regs.filter((r) => r.alumniId === alumniId),
    };
  },
};
