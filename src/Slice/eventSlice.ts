...
export const updateEventAsync = createAsyncThunk(
  "events/update",
  async (
    { id, data }: { id: string; data: UpdateEventRequest },
    { rejectWithValue },
  ) => {
...
export const registerForEventAsync = createAsyncThunk(
  "events/register",
  async (
    { eventId, alumniId }: { eventId: string; alumniId: string },
    { rejectWithValue },
  ) => {
...
export const cancelEventRegistrationAsync = createAsyncThunk(
...
export const fetchEventRegistrationsAsync = createAsyncThunk(
...
export const fetchMyRegistrationsAsync = createAsyncThunk(
...
const eventSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(registerForEventAsync.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      ...
      .addCase(cancelEventRegistrationAsync.fulfilled, (state, action) => {
        state.myRegistrations = state.myRegistrations.filter(
          (r) =>
            !(
              r.eventId === action.payload.eventId &&
              r.alumniId === action.payload.alumniId
            ),
        );
        state.loading = false;
      })
      .addCase(cancelEventRegistrationAsync.pending, (state) => {
        state.loading = true;
      })
      .addCase(cancelEventRegistrationAsync.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload as string;
      })
      .addCase(fetchEventRegistrationsAsync.fulfilled, (state, action) => {
        state.registrations = action.payload;
      })
      .addCase(fetchMyRegistrationsAsync.fulfilled, (state, action) => {
