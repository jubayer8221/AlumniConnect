...
} from "@/validation/noticeValidation";
import {
  CommonPageHeader,
  CommonInputField,
  CommonSelectField,
...
export default function NoticeFormPage() {
  ...
  useEffect(() => {
    if (isEdit && id) dispatch(fetchNoticeByIdAsync(id));
  }, [dispatch, isEdit, id]);
  useEffect(() => {
    if (isEdit && selectedNotice) {
      reset({
        ...
        publishDate: selectedNotice.publishDate,
        category: selectedNotice.category as never,
  ...
  useEffect(() => {
    if (!isEdit || !selectedNotice) return;
  ...
  const onSubmit = async (data: NoticeFormValues) => {
    if (isEdit && id) {
      const res = await dispatch(
        updateNoticeAsync({ id, data: { ...data, id } }),
      );
        ...
        <CommonCard>
          <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
            ...
            <Box sx={{ display: "flex", gap: 2, justifyContent: "flex-end" }}>
              <CommonButton variant="outlined" onClick={() => navigate(-1)}>
                Cancel
              </CommonButton>
              <CommonButton
                type="submit"
